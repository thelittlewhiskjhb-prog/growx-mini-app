...
function go(id){
  ...
  const memberPages=["dashboard","packages","wallet","recharge","withdraw","gift","security","admin"];
  const isMember = memberPages.includes(id);
  document.body.classList.toggle("logged-in", isMember);
  const nav=document.getElementById("memberNav");
  if(nav) nav.style.display=isMember ? "grid" : "none";
